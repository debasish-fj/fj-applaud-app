...
import type { CurrentWeek, ShortlistEntry, PublicNomination } from "../lib/api-types";

/* ─── Helpers ─────────────────────────────────────────────────── */

const CONFETTI_COLORS = [
  "#c8982e", "#e8c870", "#ffd700", "#daa83c",
  "#ffffff", "#f0f4f8", "#a87820",
];

const AVATAR_PALETTE_DARK = [
  { bg: "rgba(139,92,246,0.2)", border: "rgba(139,92,246,0.4)", text: "#c4b5fd" },
  { bg: "rgba(59,130,246,0.2)", border: "rgba(59,130,246,0.4)", text: "#93c5fd" },
  { bg: "rgba(16,185,129,0.2)", border: "rgba(16,185,129,0.4)", text: "#6ee7b7" },
  { bg: "rgba(249,115,22,0.2)", border: "rgba(249,115,22,0.4)", text: "#fdba74" },
  { bg: "rgba(236,72,153,0.2)", border: "rgba(236,72,153,0.4)", text: "#f9a8d4" },
];

const AVATAR_PALETTE_LIGHT = [
  { bg: "rgba(109,40,217,0.1)", border: "rgba(109,40,217,0.35)", text: "#6d28d9" },
  { bg: "rgba(37,99,235,0.1)", border: "rgba(37,99,235,0.35)", text: "#1d4ed8" },
...
function getAggregatedFeedback(nomineeId: number, nominations: PublicNomination[]) {
  ...
  return {
    combinedFeedback: filtered
      ...
      .filter(Boolean)
      .join("\n\n"),
    impact: filtered.map((n) => n.impact_note).filter(Boolean).join(" · ") || "—",
    collaboration: filtered.map((n) => n.collaboration_note).filter(Boolean).join(" · ") || "—",
    initiatives: filtered.map((n) => n.initiative_note).filter(Boolean).join(" · ") || "—",
    values: filtered.map((n) => n.values_note).filter(Boolean).join(" · ") || "—",
  };
}

/* ─── Types ─────────────────────────────────────────────────────── */

type FeedbackKey = "combinedFeedback" | "impact" | "collaboration" | "initiatives" | "values";
  ...
  return (
    ...
    >
      <Flex align="center" gap="4">
        ...
        <Box>
          <Box h="12px" w="120px" bg="var(--bdr-2)" borderRadius="6px" mb="1.5" />
...
export default function VoteClient() {
  ...
  const [publicNominations, setPublicNominations] = useState<PublicNomination[]>([]);
  ...
  const [feedbackNomineeId, setFeedbackNomineeId] = useState<number | null>(null);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const isVotingOpen =
          week.status === "VOTING_OPEN" || week.status === "SHORTLIST_PUBLISHED";
    ...
    return (
      <Box minH="100vh" color="var(--fg-1)">
        <Container maxW="2xl" px="6" pt="12" pb="2">
          <Flex align="center" gap="4">
            <Flex
              ...
              bg="var(--app-accent-muted)" borderWidth="1px"
            ...
            <Box>
              <Heading fontSize="28px" fontWeight="800" letterSpacing="-0.03em" color="var(--fg-1)" lineHeight="1.15" mb="1">
                Cast Your Vote
              </Heading>
        ...
        <Container maxW="2xl" py="6" px="6">
          <Box bg="var(--sur-1)" borderWidth="1px" borderColor="var(--bdr-1)" borderRadius="18px"
            px={{ base: "4", md: "6" }} py={{ base: "5", md: "6" }}
            style={{ boxShadow: theme === "dark" ? "0 1px 3px rgba(0,0,0,0.3)" : "0 1px 3px rgba(0,0,0,0.04)" }}
          >
  ...
  if (noActiveWeek || votingNotOpen || pageError) {
    return (
      <Box minH="100vh" color="var(--fg-1)">
        ...
        <Container maxW="2xl" py="6" px="6">
          <Box bg="var(--sur-1)" borderWidth="1px" borderColor="var(--bdr-1)" borderRadius="18px"
            px={{ base: "5", md: "8" }} py={{ base: "8", md: "10" }}
            textAlign="center"
          >
            <Text fontSize="32px" mb="4">🗳️</Text>
            ...
            <Text fontSize="sm" color="var(--fg-3)" lineHeight="1.7" maxW="320px" mx="auto">
              {pageError ??
                (noActiveWeek
            ...
            <Link href="/">
              <Button
                mt="6" h="40px" px="6" borderRadius="10px"
                variant="outline" borderColor="var(--bdr-2)"
                color="var(--fg-2)" fontSize="sm" fontWeight="600"
                _hover={{ borderColor: "var(--app-accent-border)", color: "var(--app-accent)", bg: "var(--app-accent-muted)" }}
              >
                Back to Home
              </Button>
            </Link>
  ...
  if (alreadyVoted) {
    return (
      <Box minH="100vh" color="var(--fg-1)">
        ...
        <Container maxW="2xl" py="6" px="6">
          <Box bg="var(--sur-1)" borderWidth="1px" borderColor="var(--app-accent-border)" borderRadius="18px"
            px={{ base: "5", md: "8" }} py={{ base: "8", md: "10" }}
            textAlign="center"
          >
            <Text fontSize="64px" mb="3" display="inline-block">🏆</Text>
            <Text
              fontSize="10px" fontWeight="700" color="var(--app-accent)"
              letterSpacing="0.1em" textTransform="uppercase" mb="2"
            >
              Vote Submitted
            </Text>
            <Text fontSize="20px" fontWeight="700" letterSpacing="-0.02em" color="var(--fg-1)" lineHeight="1.25" mb="3">
              Your vote has been recorded
            </Text>
            <Text fontSize="13px" color="var(--fg-2)" lineHeight="1.75" mb="7" maxW="300px" mx="auto">
              Results will be announced at the end of the week. Thanks for participating!
            </Text>
            <VStack gap="3">
              <Link href="/leaderboard" style={{ width: "100%", maxWidth: "320px" }}>
                <Button
                  w="full" h="44px"
                  bg="var(--app-accent)" color="white"
                  borderRadius="10px" fontWeight="600" fontSize="sm"
                  _hover={{ bg: "var(--app-accent-hover)", transform: "translateY(-1px)", boxShadow: "var(--shadow-accent)" }}
          ...
          <Flex align="center" gap="4">
            ...
            <Box>
              <Heading
                ...
                color="var(--fg-1)" lineHeight="1.15" mb="1"
              >
                Cast Your Vote
              </Heading>
              <Text color="var(--fg-3)" fontSize="sm" lineHeight="1.5" fontWeight="500">
      ...
      <Container maxW="2xl" py="6" px="6">
        ...
        >
          ...
          >
            {/* Top accent line */}
            <Box
              position="absolute" top={0} left="10%" right="10%" h="1px" pointerEvents="none"
              style={{
                background: "linear-gradient(90deg, transparent, var(--app-accent-border) 40%, var(--app-accent) 50%, var(--app-accent-border) 60%, transparent)",
                opacity: 0.6,
              }}
            />
              ...
              >
                ...
                >
                  {candidates.map((candidate, idx) => {
                    const isSelected = selected === String(candidate.nominee_id);
                    ...
                    return (
                      ...
                      >
                        <Box
                          borderWidth="1px"
                          borderColor={isSelected ? "rgba(200,152,46,0.5)" : "var(--bdr-1)"}
                          borderRadius="14px"
                          px="5"
                          py="4"
                          bg={
                            ...
                            >
                              <HStack gap="4" minW={0}>
                                ...
                                <Flex
                                  ...
                                  bg={avatar.bg} borderWidth="1px" borderColor={avatar.border}
                                  align="center" justify="center" flexShrink={0}
                                >
                                  <Text
                                    fontSize="xs" fontWeight="700"
                                    color={avatar.text} letterSpacing="0.05em"
                                  >
                                    {initials}
                                  </Text>
                                </Flex>
                                <Box minW={0}>
                                  <Text fontWeight="600" fontSize="sm" color="var(--fg-1)" lineHeight="1.3">
                                    {fullName}
                                  </Text>
                                  <Text fontSize="xs" color="var(--fg-4)" mt="0.5" fontWeight="500">
                                    Rank #{candidate.rank}
                                  </Text>
                                </Box>
                              </HStack>
            ...
            {candidates.length > 0 && (
              ...
              >
                {submitError && (
                  <Text fontSize="xs" color="red.400">{submitError}</Text>
                )}
                <Button
                  bg="var(--app-accent)"
                  color="white"
                  ...
                  <VStack align="stretch" gap="5">
                    ...
                    <Flex align="center" gap="4" pb="1">
                      <Flex
                        ...
                        bg={feedbackAvatar.bg} borderWidth="1px" borderColor={feedbackAvatar.border}
                        align="center" justify="center" flexShrink={0}
                      >
                        <Text
                          fontSize="sm" fontWeight="700"
                          color={feedbackAvatar.text} letterSpacing="0.05em"
                        >
                      ...
                      <Box>
                        ...
                        <Heading fontSize="18px" fontWeight="700" letterSpacing="-0.02em" color="var(--fg-1)">
                          {feedbackCandidate.first_name} {feedbackCandidate.last_name}
                        </Heading>
                      </Box>
                    </Flex>
                    ...
                    {!feedback && (
                      <Text color="var(--fg-3)" fontSize="sm" textAlign="center" py="4">
                    ...
                    {feedback && (
                      <>
                        {/* Quote block */}
                        <Box
                          bg={theme === "dark" ? "var(--sur-2)" : "rgba(0,0,0,0.02)"}
                          borderWidth="1px"
                          borderColor="var(--app-accent-border)"
                          borderLeftWidth="3px"
                          ...
                          py="4"
                        >
                          <Text color="var(--fg-1)" fontSize="sm" lineHeight="1.85" fontWeight="400"
                            style={{ whiteSpace: "pre-line" }}
                          >
                            {feedback.combinedFeedback || "No justification provided."}
                          </Text>
                        </Box>
                        ...
                        <Box display="grid" gridTemplateColumns="repeat(2, 1fr)" gap="3">
                          {FEEDBACK_SECTIONS.map(({ key, label }) => (
                            ...
                            >
                              ...
                              <Text color="var(--fg-1)" fontSize="xs" lineHeight="1.65">
                                {feedback[key]}
                              </Text>
                            </Box>
                          ))}
          ...
          >
            ...
            >
              ...
              <motion.div
                ...
                style={{ fontSize: "68px", marginBottom: "8px", display: "inline-block" }}
              >
                🏆
              </motion.div>
              ...
              >
                <Flex justify="center" mb="4">
                  <Flex
                    w="52px" h="52px" borderRadius="13px"
                    bg={selectedAvatar.bg} borderWidth="1px" borderColor={selectedAvatar.border}
                    align="center" justify="center"
                  >
                    <Text fontSize="sm" fontWeight="700" color={selectedAvatar.text} letterSpacing="0.05em">
                      {selectedCandidate
                ...
                <Text
                  ...
                  color="var(--fg-1)" lineHeight="1.25" mb="3"
                >
                  You voted for
                  <br />
                  {selectedCandidate
                ...
                <Text fontSize="13px" color="var(--fg-2)" lineHeight="1.75" mb="7">
                  Your vote has been recorded. Results will be announced at the end of the week.
                </Text>
                ...
                <VStack gap="3">
                  ...
                  <Button
                    w="full" h="40px" variant="ghost"
                    color="var(--fg-4)" borderRadius="10px" fontSize="sm"
